...
- A product's `StockMovement` rows carry no price of their own; `models/StockMovement.js` prices `SALE`/`PURCHASE_RECEIPT`/`PURCHASE_RETURN` rows on the way out by batch-looking up the source line's subtotal (`TransactionItem`/`ReceivingReportItem`/`PurchaseReturnItem`, keyed by `referenceType:referenceId:productId` — see `attachAmounts`/`loadAmounts`), so the frontend's "Stock History" modal (the per-product sales ledger) can show an Amount column and export the full unpaginated ledger (`GET /api/products/:id/movements/export`, capped at 20000 rows) without a schema change.
- `TransactionModel.findForReport` (`GET /api/sales-report?month=YYYY-MM`, `ROLES.FINANCE`) returns one row per transaction for a store-local calendar month (default: the current month) plus a `totals` summary, for the Sales Report page. Like `loadForecastInput`, it fetches a coarse UTC window and filters precisely with `localDate`/`STORE_TIMEZONE` from `models/DemandForecast.js` rather than computing timezone-exact SQL bounds.
- A Receiving Report's `items` aren't limited to what's on its Purchase Order: a line is either `{ productId, quantity, unitCost }` (a PO line, or an "extra" existing product not on the PO — only the former is capped by the ordered quantity) or `{ newProduct: { name, price }, quantity, unitCost }`, which creates the product inside the same transaction (category/unit/min stock left at schema defaults, barcode auto-generated the same way the "Add Product" form does, `supplierId` set to the PO's supplier) before writing the receiving line. Either way, the PO itself is never modified — it still reflects only what was originally ordered — and `ReceivingReportModel.create` writes every line the same way, so returnable-quantity capping (`withReturnableQuantities`) already covers extras and new products without any special-casing. The frontend's `ReceivingReportModal.jsx` collects these as a separate "extra items" list, picked or typed through `ProductCombobox.jsx` (the same searchable-combobox pattern as `SupplierCombobox.jsx`, but light-themed and matching by product name/barcode).
- A Purchase Return starts from a supplier, not a single Receiving Report: `receivingReportId` lives on `PurchaseReturnItem` (not `PurchaseReturn`), so one return's lines can each be capped against — and draw their `unitCost` from — a different delivery batch of that same supplier. `PurchaseReturnModel.create` locks every distinct receiving report a return touches (ascending id order, so overlapping concurrent returns can't deadlock each other), validates each one actually belongs to the given supplier, and caps each `(receivingReportId, productId)` line independently via the same returnable-quantity logic `withReturnableQuantities` uses. Stock is still decremented once per *product* even when a return pulls that product from more than one batch — quantities are summed across that product's lines before calling `changeStock` — so each `PURCHASE_RETURN` `StockMovement` row's attached amount (see `attachAmounts` above) stays exact instead of every row for that product sharing one summed value. The frontend's `PurchaseReturnModal.jsx` picks a supplier first, then shows every receiving report they've delivered as its own grouped, checkable batch.
- Route handlers consistently: wrap logic in try/catch, `console.error` on failure, and return `res.status(500).json({ error: '...' })`.

### Frontend
- Routing is centralized in `frontend/src/App.jsx` using `react-router-dom`. `/pos` (cashier-facing) and `/` (login) render standalone; everything else (`/adminDashboard`, `/inventoryList`, `/pages/ims/demand`, `/pages/ims/finance`, `/pages/ims/salesReport`) is nested under an `AppLayout` route element that renders the shared `Sidebar` plus an `<Outlet />`. Every route except `/` is wrapped in `<RequireAuth>` (`src/auth/RequireAuth.jsx`), which redirects to `/` when `AuthContext` has no session.
...
- Socket.IO client wiring and export helpers (`src/utils/exportCsv.js`, and `src/utils/exportExcel.js` for the shared ExcelJS-based `exportToExcel(data, fileName, sheets?)` used by the inventory sheet, Stock History ledger, purchasing docs, and Sales Report exports) are the other cross-cutting frontend concerns; charts are built with `recharts`.
