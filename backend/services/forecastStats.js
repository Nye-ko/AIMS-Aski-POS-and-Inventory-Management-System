...
const SERVICE_Z = 1.6449; // one-sided 95%: safety stock covers demand 19 times out of 20
...
// Standard deviation of the total sold over `days` days at `rate` per day. Day-to-day noise adds up with
...
const totalSd = (rate, levelDays, variance, days, countData = false) => {
  let v = variance !== null ? variance : countData ? rate : 0;
  if (countData && rate > v) v = rate;
  return Math.sqrt(days * v + (days * days * v) / levelDays);
};
...
const rangeForTotal = (rate, levelDays, variance, days, countData = false) => {
  const sd = totalSd(rate, levelDays, variance, days, countData);
  const total = rate * days;
  return [Math.max(0, total - INTERVAL_Z * sd), total + INTERVAL_Z * sd];
};

module.exports = { windowMean, sampleVariance, totalSd, rangeForTotal, INTERVAL_Z, SERVICE_Z, VARIANCE_WINDOW_DAYS };
